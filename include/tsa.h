 ...
 * depending on the perpendicular distance of the points and epsilon, the
 * greater epsilon, ore points are deleted.
 *
 * @param epsilon It acts as  the threshold value to decide which points should
 * be considered meaningful or not.
 *
 * @return array Array with the same dimensions as tss where the time series
 * have been adjusted for zero mean and one as standard deviation.
 */
std::vector<tsa::simplification::Point>
ramerDouglasPeucker(std::vector<tsa::simplification::Point> pointList,
                    double epsilon);
...
/**
 * @brief Simplifies a set of points by applying the visvalingam method (minimun
 * triangle area) until the number of points is reduced to @num_points_allowed.
 *
 * @param pointList Expects an input array whose dimension zero is the length of
 * the time series.
 *
 * @param numPoints Sets the number of points in pointList after the execution
 * of the method.
 *
 * @return a vector with the same dimensions as poinList where the number of
 * points has been reduced up to numPoints.
 */
std::vector<tsa::simplification::Point>
visvalingam_simplify(std::vector<tsa::simplification::Point> pointList,
                     int numPoints);
 ...
 * series.
 *
 * @param epsilon Minimum standard deviation to consider.  It acts a a
 * gatekeeper for those time series that may be constant or near constant.
 *
 * @return af::array Array with the same dimensions as tss where the time series
 * have been adjusted for zero mean and one as standard deviation.
 */
...
/**
 * @brief Normalizes the given time series according to its minimum and maximun
 * value and adjusts each value within the range [low, high]
 *
 * @param tss Expects an input array whose dimension zero is the length of the
 * time series (all the same) and dimension one indicates the number of time
 * series.
 ...
 *
 * @param epsilon Safeguard for constant (or near constant) time series as the
 * operation implies a unit scale operation between min and max values in the
 * tss.
 *
 * @return af::array An array with the same dimensions as tss, whose values
 * (time series in dimension 0) have been normalized by maximun and minimun
 * values, and scaled as per high and low parameters.
 */
af::array maxMinNorm(af::array tss, double high = 1.0, double low = 0.0,
                     double epsilon = 0.00000001);

/**
 * @brief Same as maxMinNorm, but it performs the operation in place, without
 ...
 * tss.
 */
void maxMinNormInPlace(af::array &tss, double high = 1.0, double low = 0.0,
                       double epsilon = 0.00000001);
 ...
 *
 * @return af::array An array with the same dimensions as tss, whose values
 * (time series in dimension 0) have been normalized by dividing each number by
 * 10^j, where j is the number of integer digits of the max number in the
 * timeseries
 */
af::array decimalScalingNorm(af::array tss);

/**
 * @brief Same as decimalScalingNorm, but it performs the operation in place,
 ...
 *
 * @param base Expects an input array whose dimension zero is the length of the
 * time series (all the same) and dimension one indicates the number of time
 * series.
 * @return af::array An array with the same dimensions as tss, whose values
 * (time series in dimension 0) contains the sum of the suares values in the
 * timeseries
 */
...
// tsfresh
/**
 * @brief Calculates the sum over the absolute value of consecutive changes in
 * the time series
 *
 * @param tss Expects an input array whose dimension zero is the length of the
 * time series (all the same) and dimension one indicates the number of time
 * series.
 * @return af::array An array with the same dimensions as tss, whose values
 * (time series in dimension 0) contains absolute value of consecutive changes
 * in the time series
 */
af::array absoluteSumOfChanges(af::array tss);

/**
 * @brief Calculates a linear least-squares regression for values of the time
 * series that were aggregated over chunks versus the sequence from 0 up to the
 ...
 * @param pvalue Two-sided p-value for a hypothesis test whose null hypothesis
 * is that the slope is zero, using Wald Test with t-distribution of the test
 * statistic
 * @param stderrest Standard error of the estimated gradient
 */
void aggregatedLinearTrend(af::array t, long chunkSize,
                           af::array (*aggregationFunction)(const af::array &,
                                                            const int),
                           af::array &slope, af::array &intercept,
...
/**
 * @brief Piecewise Aggregate Approximation. It reduces the dimensionality of
 * the timeseries given by @begin and @last iterators to a number equal to bins.
 * This algorithim divides the whole timeseries in @bins partitions and computes
...
std::vector<tsa::simplification::Point> PAA_CPU(InputIt begin, InputIt last,
                                                int bins) {
  ...
  std::vector<tsa::simplification::Point> result(
      bins, tsa::simplification::Point(0.0, 0.0));
...
/**
 * @brief Symbolic Aggregate approXimation. It transforms a numeric timeseries
 * into a timeseries of symbols with the same size.
 *
 ...
 * series.
 *
 * @return array Returns an upper triangular matrix where each position
 * corresponds to the distance between two time series. Diagonal elements will
 * be zero.  For example: Position row 0 column 1 will record the distance
 ...
 * @return array Returns an array with as many elements as 't' in the first
 * dimension and as many elements as the last dimension of 'q' in the last
 * dimension
 */
af::array slidingDotProduct(af::array q, af::array t);

/**
 * @brief Calculates the moving average and standard deviation of the time
 * series 't'.
 *
 * @param t Input time series. Multiple time series
 * @param a Auxiliary array to be used in the function calculateDistanceProfile.
 * Use the overloaded method without this parameter
 * @param m Window size
 * @param mean Output array containing the moving average
 * @param stdev Output array containing the moving standard deviation
 */
void meanStdev(af::array t, af::array &a, long m, af::array &mean,
               af::array &stdev);
...
/**
 * @brief Function to generate a band matrix of batchSizeXtsLength with the
 * offset batchStart
 *
...
/**
 * @brief Calculates the distance between 'q' and the time series 't', which
 * produced the sliding. Multiple queries can be computed simultaneously in the
 * last dimension of 'q'.
 * @param m Subsequence length (required to mask the minimum m/2 positions left
 * and right in case ignoreTrivial is true)
 * @param qt The sliding dot product of 'q' and 't'
 * @param a Auxiliary array computed using the meanStdev function. This array
 * contains a precomputed fixed value to speed up the distance calculation
 * @param sum_q Sum of the values contained in 'q'
 * @param sum_q2 Sum of squaring the values contained in 'q'
 * @param mean_t Moving average of 't' using a window size equal to the number
 * of elements in 'q'
 * @param sigma_t Moving standard deviation of 't' using a window size equal to
 * the number of elements in 'q'
 * @param mask Mask band matrix to filter the trivial match of a subsequence
 * with itself
 * @param distance Resulting minimal distance
 * @param index Position where the minimum is occurring
 * @param batchStart Indicates where the currently computed batch starts.
 * Defaults to 0 for the parallel case. The parameter is used to determine the
 * mask for the trivial matches.
 */
void calculateDistanceProfile(long m, af::array qt, af::array a,
                              af::array sum_q, af::array sum_q2,
                              af::array mean_t, af::array sigma_t,
                              af::array mask, af::array &distance,
                              af::array &index);
 ...
 * @param sigma_t Moving standard deviation of 't' using a window size equal to
 * the number of elements in 'q'
 * @param ignoreTrivial Boolean value that indicates whether the function should
 * consider the trivial match of a subsequence with itself or not
 * @param distance Resulting minimal distance
 * @param index Position where the minimum is occurring
 */
void mass(af::array q, af::array t, long m, af::array a, af::array mean_t,
          af::array sigma_t, af::array mask, af::array &distance,
          af::array &index);
...
/**
 * @brief STAMP algorithm to calculate the matrix profile between 'ta' and 'tb'
 * using a subsequence length of 'm'
 *
 * @param ta Query time series
 * @param tb Reference time series
 * @param m Subsequence length
 * @param profile The matrix profile, which reflects the distance to the closer
 * element of the subsequence from 'ta' in 'tb'
 * @param index The matrix profile index, which points to where the previously
 * mentioned minimum is located
 */
void stomp(af::array ta, af::array tb, long m, af::array &profile,
           af::array &index);

/**
 * @brief STAMP algorithm to calculate the matrix profile between 't' and itself
 * using a subsequence length of 'm'. This method filters the trivial matches.
 *
 * @param t Query and reference time series
 * @param m Subsequence length
 * @param profile The matrix profile, which reflects the distance to the closer
 * element of the subsequence from 't' in a different location of itself
 * @param index The matrix profile index, which points to where the previously
 * mentioned minimum is located
 */
...
/**
 * @brief This function extracts the best N motifs from a previously calculated
 * matrix profile
 *
 ...
 * @param subsequenceIndices The indices of the query sequences that produced
 * the minimum reported in the motifs output array
 */
void findBestNMotifs(af::array profile, af::array index, long n,
                     af::array &motifs, af::array &motifsIndices,
                     af::array &subsequenceIndices);

/**
 * @brief This function extracts the best N discords from a previously
 ...
 * @param subsequenceIndices The indices of the query sequences that produced
 * the discords reported in the discords output array
 */
void findBestNDiscords(af::array profile, af::array index, long n,
                       af::array &discords, af::array &discordsIndices,
                       af::array &subsequenceIndices);
 ...
 */
void find_best_n_discords(double *profile, unsigned int *index,
                          long *length_profile, long *n,
                          double *discord_distances, int *discord_indices,
                          int *subsequence_indices);
 ...
 */
void find_best_n_motifs(double *profile, unsigned int *index,
                        long *length_profile, long *n, double *motif_distances,
 ...
 * profile.
 */
void stomp(double *ta, double *tb, int *lta, int *ltb, long *m, double *p,
           unsigned int *i);
 ...
 * the minimum reported in the motifs
 */
JNIEXPORT void JNICALL Java_tsa_TSA_findBestNMotifs(
    JNIEnv *env, jobject thisObj, jdoubleArray profile, jintArray index,
 ...
 * the "N" bigger discords.
 */
JNIEXPORT void JNICALL Java_tsa_TSA_findBestNDiscords(
    JNIEnv *env, jobject thisObj, jdoubleArray profile, jintArray index,
    jlong lengthProfile, jlong n, jdoubleArray jDiscordDistances,
    jintArray jDiscordIndices, jintArray jSubsequenceIndices);
 ...
 */
JNIEXPORT void JNICALL Java_tsa_TSA_stomp(JNIEnv *env, jobject thisObj,
                                          jdoubleArray ta, jdoubleArray tb,
                                          jint lta, jint ltb, jlong m,
 ...
 */
JNIEXPORT void JNICALL Java_tsa_TSA_stompSelfJoin(JNIEnv *env, jobject thisObj,
                                                  jdoubleArray ta, jint lta,
                                                  jlong m, jdoubleArray p,
                                                  jintArray i);
...
/**
 * @brief Calculate a linear least-squares regression for two sets of
 ...
 * @param stderrest Standard error of the estimated gradient
 */
void linear(af::array x, af::array y, af::array &slope, af::array &intercept,
