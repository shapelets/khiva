...
void tsa::matrix::meanStdev(af::array t, af::array &a, long m, af::array &mean,
                            af::array &stdev) {
  long na = t.dims(0);
  ...
  sum_t = cumulative_sum_t(af::seq(m - 1, na - 1)) -
          af::join(0, tmp, cumulative_sum_t(af::seq(0, na - m - 1)));
  sum_t2 = cumulative_sum_t2(af::seq(m - 1, na - 1)) -
           af::join(0, tmp, cumulative_sum_t2(af::seq(0, na - m - 1)));
...
void tsa::matrix::meanStdev(af::array t, long m, af::array &mean,
                            af::array &stdev) {
  long na = t.dims(0);
...
af::array tsa::matrix::generateMask(long m, long batchSize, long batchStart,
                                    long tsLength) {
  long bandSize = std::ceil(m / 2.0);

  int tmp = batchStart > 0;
  af::array mask =
      af::convolve2(af::shift(af::identity(std::max(batchSize, bandSize) + 1,
                                           tsLength + bandSize - 1),
                              0, batchStart - tmp),
                    af::constant(1, bandSize, bandSize)) > 0;
...
void tsa::matrix::calculateDistanceProfile(long m, af::array qt, af::array a,
  ...
  af::array dist =
      a_tiled +
      (-2 * (qt - sum_q_tiled * mean_t_tiled) /
       af::max(sigma_t_tiled,
               af::constant(EPSILON, sigma_t_tiled.dims(), qt.type()))) +
      sum_q2_tiled;
...
void tsa::matrix::mass(af::array q, af::array t, long m, af::array a,
                       af::array mean_t, af::array sigma_t, af::array mask,
  ...
  tsa::matrix::calculateDistanceProfile(m, qt, a, sum_q, sum_q2, mean_t,
                                        sigma_t, mask, distance, index);
}

void tsa::matrix::mass(af::array q, af::array t, long m, af::array a,
                       af::array mean_t, af::array sigma_t, af::array &distance,
                       af::array &index) {
  q = tsa::normalization::znorm(q, EPSILON);
  ...
  tsa::matrix::calculateDistanceProfile(m, qt, a, sum_q, sum_q2, mean_t,
                                        sigma_t, distance, index);
}

void stomp_batched(af::array ta, af::array tb, long m, long batch_size,
  ...
  for (long i = 0; i < (nb - m); i += chunkSize) {
    ...
    for (long j = 0; j < m; j++) {
      input(j, span, span, span) =
          tb(af::seq(i + j, i + j + iterationSize - 1));
    }
    ...
    gfor(af::seq idx, iterationSize) {
      ...
      tsa::matrix::mass(input(span, idx, span, span), ta, m, aux, mean, stdev,
                        distance, pidx);
...
void stomp_batched_two_levels(af::array ta, af::array tb, long m,
                              long batch_size_b, long batch_size_a,
                              af::array &profile, af::array &index) {
  long nb = tb.dims(0);
  long na = ta.dims(0);
  ...
  for (long i = 0; i < (nb - m); i += chunkSizeB) {
    ...
    for (long j = 0; j < m; j++) {
      input(j, span, span, span) =
          tb(af::seq(i + j, i + j + iterationSizeB - 1));
    }
    ...
    for (long k = 0; k < na; k += chunkSizeA) {
      ...
      gfor(af::seq idx, iterationSizeB) {
        ...
        tsa::matrix::mass(input(span, idx, span, span), taChunk, m, aux, mean,
                          stdev, distanceTmp, pidxTmp);
    ...
    index =
        af::join(0, index, af::moddims(vFromPidx, af::dim4(dims[0], dims[1])));
  }
}

void stomp_parallel(af::array ta, af::array tb, long m, af::array &profile,
                    af::array &index) {
  long nb = tb.dims(0);
...
void tsa::matrix::stomp(af::array ta, af::array tb, long m, af::array &profile,
                        af::array &index) {
  if (tb.dims(0) > BATCH_SIZE) {
    if (ta.dims(0) > BATCH_SIZE) {
      return stomp_batched_two_levels(ta, tb, m, BATCH_SIZE, BATCH_SIZE,
                                      profile, index);
    } else {
...
void stomp_batched_two_levels(af::array t, long m, long batch_size_b,
                              long batch_size_a, af::array &profile,
                              af::array &index) {
  long n = t.dims(0);
  ...
  for (long i = 0; i < (n - m); i += chunkSizeB) {
    ...
    for (long j = 0; j < m; j++) {
      input(j, span, span, span) =
          t(af::seq(i + j, i + j + iterationSizeB - 1));
    }
    ...
    af::array mask =
        tsa::matrix::generateMask(m, iterationSizeB, i, chunkSizeA - m + 1);
    ...
    for (long k = 0; k < n; k += chunkSizeA) {
      ...
      gfor(af::seq idx, iterationSizeB) {
        ...
        tsa::matrix::mass(input(span, idx, span, span), tChunk, m, aux, mean,
                          stdev, mask(span, af::seq(iterationSizeA - m + 1)),
                          distanceTmp, pidxTmp);
...
void stomp_parallel(af::array t, long m, af::array &profile, af::array &index) {
  ...
  gfor(af::seq idx, n - m + 1) {
    tsa::matrix::mass(input(span, idx, span, span), t, m, aux, mean, stdev,
                      mask, profile, index);
  }
  af::sync();
}
...
void tsa::matrix::stomp(array t, long m, af::array &profile, af::array &index) {
  if (t.dims(0) > BATCH_SIZE) {
    return stomp_batched_two_levels(t, m, BATCH_SIZE, BATCH_SIZE, profile,
                                    index);
  } else {
...
void tsa::matrix::findBestNMotifs(af::array profile, af::array index, long n,
                                  af::array &motifs, af::array &motifsIndices,
                                  af::array &subsequenceIndices) {
  af::array profileTiled = af::tile(profile, 1, 2);
  af::array joined =
      af::join(1, index, af::range(index.dims(0), 1, 1, 1, -1, index.type()));
...
void tsa::matrix::findBestNDiscords(af::array profile, af::array index, long n,
                                    af::array &discords,
...
void find_best_n_discords(double *profile, unsigned int *index,
  ...
  af::array subsequenceIndices;
  tsa::matrix::findBestNDiscords(array(*length_profile, profile),
                                 array(*length_profile, index), *n, discords,
...
void find_best_n_motifs(double *profile, unsigned int *index,
  ...
  af::array subsequenceIndices;
  tsa::matrix::findBestNMotifs(array(*length_profile, profile),
                               array(*length_profile, index), *n, motifs,
...
void stomp_self_join(double *ta, int *lta, long *m, double *p,
                     unsigned int *i) {
  af::array distance;
  af::array index;
...
JNIEXPORT void JNICALL Java_tsa_TSA_findBestNDiscords(
  ...
  tsa::matrix::findBestNDiscords(array(lengthProfile, inputP),
                                 array(lengthProfile, inputI), n, discords,
                                 discordIndices, subsequenceIndices);
  ...
  env->SetIntArrayRegion(jSubsequenceIndices, 0, n,
                         &inputSubsequenceIndices[0]);
...
JNIEXPORT void JNICALL Java_tsa_TSA_findBestNMotifs(
  ...
  tsa::matrix::findBestNMotifs(array(lengthProfile, inputP),
                               array(lengthProfile, inputI), n, motifs,
                               motifIndices, subsequenceIndices);
