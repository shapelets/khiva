...
af::array tsa::normalization::decimalScalingNorm(af::array tss) {
    ...
    auto result = tss / af::tile(divFactor, static_cast<const unsigned int>(tss.dims(0)));
    return result;
}
...
void tsa::normalization::decimalScalingNormInPlace(af::array &tss) {
    ...
    auto d = af::ceil(af::log10(maxAbs));
    auto divFactor = af::pow(const10, d);
    tss /= af::tile(divFactor, static_cast<const unsigned int>(tss.dims(0)));
}

af::array tsa::normalization::maxMinNorm(af::array tss, double high, double low, double epsilon) {
    auto max = af::tile(af::max(tss, 0), static_cast<const unsigned int>(tss.dims(0)));
    auto min = af::tile(af::min(tss, 0), static_cast<const unsigned int>(tss.dims(0)));
    auto scale = max - min;
...
af::array tsa::normalization::meanNorm(af::array tss) {
    auto max = af::tile(af::max(tss, 0), static_cast<const unsigned int>(tss.dims(0)));
    auto min = af::tile(af::min(tss, 0), static_cast<const unsigned int>(tss.dims(0)));
    auto mean = af::tile(af::mean(tss, 0), static_cast<const unsigned int>(tss.dims(0)));
...
af::array tsa::normalization::znorm(af::array tss, double epsilon) {
    ...
    stdev = af::tile(lessThanEpsilon * lessThanEpsilon.as(tss.type()) + !lessThanEpsilon * stdev,
                     static_cast<const unsigned int>(tss.dims(0)));
