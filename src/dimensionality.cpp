...
std::vector<float> computeBreakpoints(int alphabet_size, float mean_value,
                                      float std_value)
{
...
std::vector<int> tsa::dimensionality::SAX(array a, int alphabet_size)
{
  ...
  std::vector<float> breakingpoints =
      computeBreakpoints(alphabet_size, mean_value, std_value);
...
void getSegmentOfPoint(float point, std::vector<float> desired_x, int &i_lower, int &i_upper){
 ...
 void insertPointInDesiredList(float x, float y, int position, std::vector<float> &desired_x, std::vector<float> &desired_y){
    ...
    for(int p = 0; p < (numberIPs - 2); p++){
        ...
        for(size_t i = 1; i < end; i++){
            ...
            if (!isPointInDesiredList(points_x[i], desired_x)){
                ...
                float d = verticalDistance(points_x[i], points_y[i], desired_x[i_lower], desired_y[i_lower], desired_x[i_upper], desired_y[i_upper]);
                if (d > dmax){
                    index = i;
                    dmax = d;
                    position = i_upper;
